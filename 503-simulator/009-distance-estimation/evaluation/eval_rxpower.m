...
for cm=1:size(channel_models,1)
  ...
  xlabel("Dist (m)");
  ylabel("PDR");
  saveas(h1, strcat(num2str(cm),'_pdr.eps'),'epsc');
  %saveas(h1, strcat(num2str(cm),'_pdr.png'),'png');
  ...
  xlabel("Dist (m)");
  ylabel("SNR (db)");
  saveas(h1, strcat(num2str(cm),'_snr.eps'),'epsc');
  %saveas(h1, strcat(num2str(cm),'_snr.png'),'png');
  
  h1=figure;
  scatter(snr_mean(:,cm),pdr_result(:,cm));
  xlabel("SNR (db)");
  ylabel("PDR");
  saveas(h1, strcat(num2str(cm),'_scatter.eps'),'epsc');
  %saveas(h1, strcat(num2str(cm),'_scatter.png'),'png');

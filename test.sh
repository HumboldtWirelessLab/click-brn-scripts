...
for next_sim in $SIMULATOR; do
  ...
  SIMULATOR=$next_sim LATEX=$ENABLE_LATEX VALGRINDOPTION="$VALGRINDOPTION" VALGRIND=$VALGRIND_PARAMS VALGRINDXML=$VALGRINDXML_PARAMS MODE=SIMULATION START=$START LIMIT=$LIMIT ./tools/testing/gen_test_pdf.sh

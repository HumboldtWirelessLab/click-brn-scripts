...
echo "RATEINDEX BANDWIDTH SGI GF FEC SHORTPREAMBLE STBC CHANNEL RX_ALL RX_OK RX_ERR" > $RESULTDIR/result.txt 
...
while [ -e $RESULTDIR/$DIRNUM ]; do
  ...
  if [ -f $RESULTDIR/$DIRNUM/params ]; then
    ...
    echo "$PARAMS_RATEINDEX $PARAMS_SGI $PARAMS_GF $PARAMS_FEC $PARAMS_SHORTPREAMBLE $PARAMS_STBC $PARAMS_CHANNEL $RX_NUM $RX_OK $RX_ERR" >> $RESULTDIR/result.txt
  fi

      ...
      for d in $DEVICES; do
        ...
        if [ ! -f $RESULTDIR/$DIRNUM/$n.$d.raw.out ]; then
          ...
          if [ -f $RESULTDIR/$DIRNUM/$n.$d.raw.dump ]; then
            ( cd $RESULTDIR/$DIRNUM; HT=true EVM=true RX=true WIFI=802 fromdump.sh $n.$d.raw.dump | grep -v "TXFeedback" | grep data | grep ":[[:space:]]*50[[:space:]]*|" | grep FF-FF-FF-FF-FF-FF | sed -e "s#Mb##g" -e "s#+[0]*##g" -e "s#/# #g" -e "s#:##g" > $n.$d.raw.out )
          else

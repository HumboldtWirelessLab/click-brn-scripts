...
INTERVAL="25"
PACKETCOUNT="10000"
RATES="b1 n1 n2 n3 n4 n5 n6 n7 n8"
...
for p_repeat in `seq $REPEAT`; do
 ...
 for p_c in $CHANNEL; do
  ...
  for p_node in `cat nodes.all`; do

    if [ ! -e $MEASUREMENT_NUM ]; then

      cat nodes.all | grep -v "$p_node" > nodes

      RUNTIME=`expr $INTERVAL \* $PACKETCOUNT`
      RUNTIME=`expr $RUNTIME / 900`
      ...
      if [ $p_c -lt 15 ]; then
        POWER=27
        cat monitor.802.tmpl |  sed -e "s#PARAMS_CHANNEL#$pc#g" -e "s#PARAMS_POWER#$POWER#g" > monitor.802
	...
	echo "Idle() -> ee_b1;" >> probe_sender.click
      ...
      TESTONLY=1 RUNMODE=$CURRENT_RUNMODE run_measurement.sh sender_and_receiver.des $MEASUREMENT_NUM

      echo "PARAMS_CHANNEL=$p_c" > $MEASUREMENT_NUM/params
      echo "PARAMS_REPEAT=$p_c" >> $MEASUREMENT_NUM/params

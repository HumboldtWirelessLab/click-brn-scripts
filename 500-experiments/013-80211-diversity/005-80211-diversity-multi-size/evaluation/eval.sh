        ...
        for d in $DEVICES; do
          ...
          if [ ! -f $RESULTDIR/$DIRNUM/$n.$d.raw.out ]; then
            ...
            if [ -f $RESULTDIR/$DIRNUM/$n.$d.raw.dump ]; then
              ...
              if [ -f $RESULTDIR/$DIRNUM/$n.$d.raw.dump.bz2 ]; then
                ( cd $RESULTDIR/$DIRNUM; CONTENT=yes HT=true EVM=true RX=true WIFI=802 $DIR/fromdump.sh $n.$d.raw.dump.bz2 | grep data | sed "s# err # #g" | sed "s# tx # #g" | sed "s#retries 0##g" | grep FF-FF-FF-FF-FF-FF | grep $SENDER_MAC | sed -e "s#Mb##g" -e "s#+[0]*##g" -e "s#/# #g" -e "s#:##g" > $n.$d.raw.out )
              else
          ...
          if [ "$n" = "$PARAMS_SENDER" ]; then
            cat $RESULTDIR/$DIRNUM/$n.$d.raw.out | grep "|  0" | awk -v NODE=$n -v SENDER=$SENDER_MAC -v REPEAT=$PARAMS_REPEAT -v CHANNEL=$PARAMS_CHANNEL '{ print $2" "$3" "SENDER" "NODE" "$1" "$5" "$6" "$7" "$8" "$9" "$21" "$22" "$30" "$10" "$11" "$13" "$14" "strtonum("0x"$39)" "REPEAT" "CHANNEL""}' | sed "s#OKPacket#1#g" | sed "s#CRCerror#0#g" | sed "s#TXFeedback#2#g" | mac_to_num $RESULTDIR/$NODESMACDIR/nodes.mac $2 | grep -v "[A-F0-9]-" >> $RESULTDIR/result_$DIRNUM\.txt
          else
            cat $RESULTDIR/$DIRNUM/$n.$d.raw.out | grep "| 130.0" | awk -v NODE=$n -v SENDER=$SENDER_MAC -v REPEAT=$PARAMS_REPEAT -v CHANNEL=$PARAMS_CHANNEL '{ print $2" "$3" "SENDER" "NODE" "$1" "$5" "$6" "$7" "$8" "$9" "$21" "$22" "$30" "$10" "$11" "$13" "$14" "strtonum("0x"$39)" "REPEAT" "CHANNEL""}' | sed "s#OKPacket#1#g" | sed "s#CRCerror#0#g" | sed "s#TXFeedback#2#g" | mac_to_num $RESULTDIR/$NODESMACDIR/nodes.mac $2 | grep -v "[A-F0-9]-" >> $RESULTDIR/result_$DIRNUM\.txt
          fi

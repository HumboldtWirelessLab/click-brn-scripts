...
for {set i 1} {$i < $nodecount } {incr i} {
    ...
    # script.
    #
[$node_($i) entry] loadclick "simple_flow_sink_out.click"
}

[$node_(0) entry] loadclick "simple_flow_source_out.click"
...
for {set i 0} {$i < $nodecount} {incr i} {
    $node_($i) set X_ [expr $i % 100 * 50 ]
    $node_($i) set Y_ [expr $i / 100 * 50 ]
    $node_($i) set Z_ 0

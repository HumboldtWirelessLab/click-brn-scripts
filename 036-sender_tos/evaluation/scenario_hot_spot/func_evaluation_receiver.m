function func_evaluation_receiver(matrix,number,packet_size,mac_address_source,number_of_nodes,packet_counter,backoff_window_size_min,rssi_value)
...
for p=1:1:size(matrix_evaluation_per_neighbour_backoff_no_packets_rx_avg,1)
    for t=2:1:size(matrix_evaluation_per_neighbour_backoff_no_packets_rx_avg,2)  
        ...
        figure_number = figure_number + 1;
        str_title = sprintf('Paketgröße = %d, max. sendende Knoten = %d',vector_packet_sizes(p,1),vector_neighbour_total(1,t-1)); 
        func_figure_neighbours_packets_rx_per_backoff(figure_number,vector_neighbours,matrix_1, vector_backoff,str_title);
        figure_number = figure_number + 1;
